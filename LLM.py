...
class VRGDG_NanoBananaPro:
    ...
    def _target_dimensions(self, resolution: str, aspect_ratio: str) -> tuple[int, int]:
        return self.RESOLUTION_DIMENSIONS.get(resolution, self.RESOLUTION_DIMENSIONS["HD"]).get(
            aspect_ratio,
            self.RESOLUTION_DIMENSIONS["HD"]["16:9"],
        )

    def _resize_to_target(self, pil_img: Image.Image, resolution: str, aspect_ratio: str) -> Image.Image:
        width, height = self._target_dimensions(resolution, aspect_ratio)
        return ImageOps.fit(pil_img.convert("RGB"), (width, height), method=Image.Resampling.LANCZOS)
    ...
    def generate(
        ...
        width, height = self._target_dimensions(resolution, aspect_ratio)
        orientation_text = "landscape" if aspect_ratio == "16:9" else "portrait"

        full_prompt = (
            f"Generate ONE {orientation_text}-only image in a strict {aspect_ratio} aspect ratio. "
            f"Target final resolution is {width}x{height} ({resolution}). "
            "Do not return any other aspect ratio.\n\n"
            f"Prompt: {prompt}"
        )
        ...
        if pil_img is not None:
            pil_img = self._resize_to_target(pil_img, resolution, aspect_ratio)
...
class VRGDG_Qwen35:
    ...
    def _format_model_load_error(
        ...
        model_id_low = str(model_id or "").lower()

        if (
            "gemma-4" in model_id_low
            and (
                "gemma4" in low
                or "unknown model type" in low
                or "unrecognized configuration class" in low
                or "autoimageprocess" in low
                or "automodelforimagetexttotext" in low
                or "processor" in low
                or "image processor" in low
                or "video processor" in low
                or "feature extractor" in low
                or "does not recognize this architecture" in low
            )
        ):
            return (
                f"Gemma 4 support is not available in the installed Transformers build for '{model_id}'.\n"
                "Update transformers to a recent version that includes Gemma 4 support, then retry."
            )
    ...
    def _strip_thinking_text(self, text: str) -> str:
        ...
        if re.search(r"<channel\|>", cleaned, flags=re.IGNORECASE):
            cleaned = re.split(r"<channel\|>", cleaned, maxsplit=1, flags=re.IGNORECASE)[-1]
        # Remove explicit reasoning headers if the model emits them verbatim.
        cleaned = re.sub(r"(?is)^\s*de_thought\s*", "", cleaned)
        ...
        cleaned = re.sub(r"(?is)^\s*thought\s*:\s*", "", cleaned)
        cleaned = re.sub(r"(?is)^\s*thought\s*$", "", cleaned)
        ...
        cleaned = re.sub(r"(?is)^\s*\*+\s*thought\s*:?\s*\*+\s*", "", cleaned)
        cleaned = re.sub(r"(?is)</?thought>", "", cleaned)
        while True:
            previous = cleaned
            cleaned = re.sub(r"(?is)^\s*de_thought(?:_[a-z0-9]+)*\s*", "", cleaned)
            cleaned = re.sub(r"(?is)^\s*thought(?:_[a-z0-9]+)*\s*", "", cleaned)
            cleaned = re.sub(r"(?is)^\s*<\|?channel\|?>\s*", "", cleaned)
            cleaned = re.sub(r"(?is)^\s*<channel\|?>\s*", "", cleaned)
...
class VRGDG_Qwen25(VRGDG_Qwen35):
    ...
    def _enforce_preset_output(self, task_preset: str, text: str) -> str:
        ...
        out = self._strip_thinking_text(text)
        if not out:
            return out
...
class VRGDG_GeneralVLM(VRGDG_Qwen25):
    ...
    def _extra_model_kwargs(self, model_id: str, task: str) -> dict:
        ...
        if "gemma-4" in mid:
            # Hugging Face Gemma 4 examples explicitly use SDPA.
    ...
    def generate_prompt(
        ...
        **kwargs,
        ...
        try:
            ...
            if (bool(unload_after_run) or not completed_ok) and model_path:
...
class VRGDG_LlamaCppPythonInstaller:
    INSTALL_MODES = [
        "default_cpu_pypi",
        "cuda_12_1",
        "cuda_12_2",
    ...
    CUDA_WHEEL_INDEXES = {
        "cuda_12_1": "https://abetlen.github.io/llama-cpp-python/whl/cu121",
        "cuda_12_2": "https://abetlen.github.io/llama-cpp-python/whl/cu122",
        "cuda_12_3": "https://abetlen.github.io/llama-cpp-python/whl/cu123",
        "cuda_12_4": "https://abetlen.github.io/llama-cpp-python/whl/cu124",
        "cuda_12_5": "https://abetlen.github.io/llama-cpp-python/whl/cu125",
    }
        ...
        return {
            "required": {
                "install_now": (
                    ...
                    {
                        "default": False,
                        "tooltip": "When enabled and queued, install llama-cpp-python into the Python environment currently running ComfyUI.",
                    },
                ),
                "install_mode": (
                    cls.INSTALL_MODES,
                    {
                        "default": "default_cpu_pypi",
                        "tooltip": "Default uses PyPI. CUDA modes use the llama-cpp-python CUDA wheel index for that CUDA version.",
                    },
                ),
                "upgrade_reinstall": (
                    "BOOLEAN",
                    {
                        "default": False,
                        "tooltip": "Add --upgrade --force-reinstall to the pip install command.",
                    },
                ),
            }
        }
    ...
    @staticmethod
    def _check_llama_cpp() -> tuple[bool, str]:
        try:
            import llama_cpp
            version = getattr(llama_cpp, "__version__", "unknown")
            return True, f"llama-cpp-python is already importable. Version: {version}"
        except Exception as e:
            return False, f"llama-cpp-python is not importable yet: {e}"

    def run(
        self,
        install_now: bool,
        install_mode: str,
        upgrade_reinstall: bool,
    ) -> Tuple[str]:
        is_available, check_message = self._check_llama_cpp()
        ...
        if not install_now:
            return (
                f"{check_message}\n\n"
                f"ComfyUI Python:\n{python_exe}\n\n"
                "Enable install_now and queue this node to run pip install llama-cpp-python.",
            )
        ...
        wheel_index = self.CUDA_WHEEL_INDEXES.get(str(install_mode or ""))
        if wheel_index:
            command.extend(["--extra-index-url", wheel_index])
        ...
        is_available_after, after_message = self._check_llama_cpp()
        restart_note = (
            "Restart ComfyUI before using the GGUF node."
            if is_available_after or result.returncode == 0
            else "Install did not complete successfully. Check the pip output below."
        )
        ...
        if output:
            status.extend(["", "pip stdout:", output[-4000:]])
        if error_output:
            status.extend(["", "pip stderr:", error_output[-4000:]])
...
NODE_CLASS_MAPPINGS = {
    ...
    "VRGDG_LlamaCppPythonInstaller": VRGDG_LlamaCppPythonInstaller,
}
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "VRGDG_LlamaCppPythonInstaller": "🛠️ VRGDG Install llama-cpp-python 🛠️",
}
