...
class VRGDG_LTXLoraTrainChunk:
    ...
    def run(
        ...
        python_exe = os.path.join(musubi_root, ".venv", "Scripts", "python.exe")
        accelerate_exe = os.path.join(musubi_root, ".venv", "Scripts", "accelerate.exe")
        if not os.path.isfile(python_exe):
            raise ValueError(f"Python executable not found: {python_exe}")
        if not os.path.isfile(accelerate_exe):
            raise ValueError(f"Accelerate executable not found: {accelerate_exe}")
...
class VRGDG_LTXPreviewXYZPlot:
    ...
    def _build_filter_complex(self, video_paths, cell_width, cell_height, columns, label_tiles, output_fps):
