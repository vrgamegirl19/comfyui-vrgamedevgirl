...
def load_timestamp_extractor_harness():
    ...
    method_names = {
        ...
        "_reference_scene_word_segments",
...
class ExactReferenceLineTimingTests(unittest.TestCase):
    ...
    def test_existing_scene_mode_uses_acoustic_word_timing_not_forced_line_stretching(self):
        units = [
            {"type": "vocal", "text": "I'll just lay here"},
            {"type": "vocal", "text": "Take your trophy"},
            {"type": "vocal", "text": "Take your glory"},
            {"type": "vocal", "text": "What a good guy"},
        ]
        raw_transcription = [
            {
                "words": [
                    ...
                    {"start": 16.16, "end": 16.5, "text": "Take"},
                    ...
                    {"start": 35.55, "end": 35.85, "text": "What"},
                    {"start": 35.9, "end": 36.0, "text": "a"},
                    {"start": 36.0, "end": 36.3, "text": "good"},
                    {"start": 36.3, "end": 36.8, "text": "guy"},
                ]
            }
        ]

        aligned = self.extractor._reference_scene_word_segments(units, raw_transcription, 45.0)

        self.assertEqual([word["text"] for word in aligned[0]["words"]], ["I'll", "just", "lay", "here"])
        self.assertAlmostEqual(aligned[0]["words"][-1]["end"], 11.95)
        self.assertAlmostEqual(aligned[2]["words"][-1]["start"], 20.14)
        self.assertAlmostEqual(aligned[3]["words"][0]["start"], 35.55)
        self.assertEqual(aligned[3]["timing_source"], "acoustic_transcription")

    def test_missing_first_word_stays_with_its_line_instead_of_filling_prior_silence(self):
        units = [
            {"type": "vocal", "text": "You patronizing son of a bitch"},
            {"type": "vocal", "text": "He said better calm down"},
        ]
        raw_transcription = [
            {
                "words": [
                    {"start": 99.76, "end": 99.86, "text": "You"},
                    {"start": 99.86, "end": 101.14, "text": "patronizing"},
                    {"start": 101.14, "end": 102.5, "text": "son"},
                    {"start": 102.5, "end": 102.8, "text": "of"},
                    {"start": 102.8, "end": 102.98, "text": "a"},
                    {"start": 103.0, "end": 103.46, "text": "bitch"},
                    # Whisper missed "He" and did not recognize the next word
                    # until the following scene.
                    {"start": 107.58, "end": 108.36, "text": "said"},
                    {"start": 108.54, "end": 109.58, "text": "better"},
                    {"start": 109.58, "end": 109.98, "text": "calm"},
                    {"start": 109.98, "end": 110.66, "text": "down"},
                ]
            }
        ]

        aligned = self.extractor._reference_scene_word_segments(units, raw_transcription, 120.0)
        he_word = aligned[1]["words"][0]

        self.assertEqual(he_word["text"], "He")
        self.assertGreater(he_word["start"], 107.14)
        self.assertAlmostEqual(he_word["end"], 107.58)
        self.assertLess(he_word["end"] - he_word["start"], 0.5)
