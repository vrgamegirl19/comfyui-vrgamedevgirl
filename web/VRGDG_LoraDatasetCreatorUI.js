...
function openCreator() {
  ...
  const llmRunner = select([["gemma_local", "Gemma Local / Gemma 4"], ["lm_studio", "LM Studio"], ["llm_api", "LLM API"]], "gemma_local"), gemmaModel = select([], ""), gemmaMmproj = select([], ""), lmUrl = input("http://127.0.0.1:1234/v1"), lmModel = select([], ""), refreshLmModels = button("Refresh LM Studio Models"), provider = select([["openai", "OpenAI"], ["anthropic", "Anthropic"], ["google", "Google Gemini"], ["grok", "Grok"], ["openrouter", "OpenRouter"]], "openai"), apiModel = select([], ""), apiKey = input("", "password");
